...
static char *determine_cd_target(t_cmd  *cmd,
                                 t_data *data,
                                 int    *print_path_flag,
                                 char  **path_to_free)
{
    ...
    *print_path_flag = FALSE;
    if (cmd->argv[1])
    {
        path_after_dash = handle_dash_path(cmd->argv[1], data, print_path_flag);
        if (!path_after_dash)
            return (NULL);
...
int ft_cd(t_cmd *cmd, t_data *data)
{
    ...
    if (!cmd->argv[1])
        ...
        final_target_path =
            determine_cd_target(cmd, data, &print_path_flag, &allocated_path);
